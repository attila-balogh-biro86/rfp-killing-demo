...
public class MissionSourceLoaderService{
  ...
  public MissionSource loadSource(Long sourceId, MissionSourceTypes sourceType) {
    if (MissionSourceTypes.RFP.equals(sourceType)) {
      return rfpRepository.findById(sourceId).orElseThrow(() -> new IllegalStateException("Missing RFP with id: "+sourceId));
    } else if (MissionSourceTypes.QUOTATION.equals(sourceType)) {
      return quotationRepository.findById(sourceId).orElseThrow(() -> new IllegalStateException("Missing Quotation with id: "+sourceId));
    }
